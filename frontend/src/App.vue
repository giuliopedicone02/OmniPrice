  ...
  <div class="min-h-screen bg-slate-50">
          ...
          <div class="flex items-center gap-8">
            ...
            <nav class="hidden sm:flex items-center gap-1">
              ...
              >
                ...
                Alert
                <span v-if="alertsCount > 0" class="bg-indigo-600 text-white text-xs font-bold rounded-full w-4 h-4 flex items-center justify-center leading-none">
                  {{ alertsCount }}
                </span>
              </router-link>
            </nav>
    ...
    <nav v-if="authStore.token" class="fixed bottom-0 left-0 right-0 bg-white border-t border-slate-200 flex sm:hidden z-50 shadow-lg">
      <router-link
        ...
        :class="$route.path === '/' ? 'text-indigo-600' : 'text-slate-500'"
      >
      ...
      <router-link
        ...
        :class="$route.path === '/alerts' ? 'text-indigo-600' : 'text-slate-500'"
      >
        <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
          <path stroke-linecap="round" stroke-linejoin="round" d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9" />
        </svg>
        Alert
        <span v-if="alertsCount > 0" class="absolute top-1.5 right-1/4 bg-indigo-600 text-white text-xs font-bold rounded-full w-4 h-4 flex items-center justify-center leading-none">
          {{ alertsCount }}
        </span>
      </router-link>
      <button
